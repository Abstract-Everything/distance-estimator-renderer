...
{
  const unsigned int width = static_cast<unsigned int>(framebufferObject()->width());
  const unsigned int height = static_cast<unsigned int>(framebufferObject()->height());
  Singletons::renderer.set_uniform(Uniform("globals.resolution", { width,  height }));
