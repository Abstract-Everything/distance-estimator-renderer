...
void Lexer::tokenize (std::string const &line)
{
  ...
  while (iterator != line.end())
  {
    ...
    switch (type)
    {
      ...
      {
        Token token (Token_Type::Directive, tokenize_directive (), line_number);
      ...
      case Character_Type::Keyword:
      {
        ...
        std::string string = tokenize_block ({ Character_Type::Keyword, Character_Type::Number });
      ...
      case Character_Type::Whitespace:
      {
        ...
        tokens.push_back (Token (Token_Type::Whitespace, space, line_number));
      }
      break;
    }
  }
...
Character_Type Lexer::determine_character_type(std::string::const_iterator const &iterator) const
{
  if (*iterator == '#')
...
{
  auto check_end_character = [this, types](std::string::const_iterator const &iterator)
  {
  ...
  while (iterator != end_iterator && check_end_character (iterator)) ++iterator;
...
{
  std::string number = tokenize_block ({ Character_Type::Number, Character_Type::Dot });
  if (*iterator == 'f')
  {
    number += *iterator;
    iterator++;
...
{
  std::string::const_iterator start_iterator = iterator;
  if (*iterator != '"') assert (false && "String needs to start with a \"");
...
{
  valid = false;
  errors.push_back ("Error parsing" + filepath.filename().string() +
