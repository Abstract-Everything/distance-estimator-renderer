...
{ Directive, Keyword, Punctuation, Dot, String, Number, Whitespace };
...
struct Token
{
  ...
  Token (Token_Type p_type, std::string p_string, int p_line_number) :
    type(p_type), string(p_string), line_number(p_line_number) {}
};
...
private:
  ...
  Character_Type determine_character_type (std::string::const_iterator const &iterator) const;
