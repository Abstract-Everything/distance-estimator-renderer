...
void Parser::process(std::filesystem::path const &path, bool is_implementation)
{
  ...
  if (is_vertex_shader (path))        vertex_shader_code = glsl_shader_code;
  else if (is_fragment_shader (path)) fragment_shader_code = glsl_shader_code;
}
...
void Parser::parse(bool is_implementation)
{
  if (iterator->type == Token_Type::Directive)
  {
    ...
    else if (iterator->string.find("#requires_implementation") != std::string::npos)
    {
      if (iterator->line_number != 1)
        register_error ("#requires_implementation should be present in the first line");

      if (is_implementation)
        register_error ("This is not an implementation file, use a file which implements the required functions");
    }

    else if (iterator->string.find("#vertex_shader") != std::string::npos)
    {
      if (vertex_shader_code.empty ()) include_vertex_shader();
      else register_error ("multiple vertex shaders are linked by this file.");
    }
...
bool Parser::expect_token (Token_Type expected_token, std::string const &error_message)
{
...
std::vector <Token>::const_iterator Parser::find_next_token (Token_Type const type) const
{
  std::vector <Token>::const_iterator search_iterator = iterator;
  while (search_iterator->type != type) search_iterator++;
  return search_iterator;
}

std::string Parser::skip_whitespace (std::vector<Token>::const_iterator &iterator) const
{
  std::string space;
...
std::unique_ptr <Variable> Parser::parse_variable ()
{
  std::unique_ptr <Variable> variable =
    std::make_unique <Typed_Variable<bool>> ("Invalid", std::vector <bool> { false });
  ...
  switch (type)
  {
    case Variable::Type::Boolean:  variable = create_variable <bool>         (variable_name, size); break;
    case Variable::Type::Integer:  variable = create_variable <int>          (variable_name, size); break;
    case Variable::Type::Uinteger: variable = create_variable <unsigned int> (variable_name, size); break;
    case Variable::Type::Float:    variable = create_variable <float>        (variable_name, size); break;
    case Variable::Type::Double:   variable = create_variable <double>       (variable_name, size); break;
    case Variable::Type::Struct:
      variable = std::make_unique<Struct_Variable> (variable_name, struct_types [type_name]); break;
      
    case Variable::Type::Invalid: assert (false && "Invalid variable type."); break;
  }
...
template <typename T>
std::unique_ptr<Variable> Parser::create_variable (std::string const &name, unsigned int size)
{
...
std::vector<T> Parser::parse_assignment_values (unsigned int size)
{
  ...
  {
    if (!expect_token (Token_Type::Open_Round_Brace, "Expected opening assignment value brace."))
    {
      iterator = find_next_token (Token_Type::Semicolon);
      return default_values;
    }
  ...
  for (size_t i = 0; i < size; ++i)
  {
    ...
    if (i + 1 < size)
    {
      if (!expect_token (Token_Type::Comma, "Expected a comma separating assignment values."))
      {
        iterator = find_next_token (Token_Type::Semicolon);
        return default_values;
      }
  ...
  if (should_be_brackated)
  {
    if (!expect_token (Token_Type::Close_Round_Brace, "Expected closing assignment value brace."))
    {
      iterator = find_next_token (Token_Type::Semicolon);
      return default_values;
    }
...
bool Parser::parse_value ()
{
  ...
  if      (iterator->string == "true")  return true;
  else if (iterator->string == "false") return false;
...
int Parser::find_number (bool is_float)
{
  ...
  if (negative) expect_token (Token_Type::Minus, "");

  bool found_number = is_float ?
    expect_token (Token_Type::Float, "Expected a floating point value.") :
...
void Parser::include_vertex_shader()
{
  if (!is_fragment_shader(path))
    register_error ("#vertex_shader should only be present in a fragment shader.");
...
{
  glsl_shader_code += iterator->string;
  if (!expect_token (Token_Type::Keyword, "Keyword struct should be followed by a name."))
  {
    iterator = find_next_token (Token_Type::Semicolon);
    return;
  }
  ...
  if (!expect_token (Token_Type::Open_Curly_Brace, "Struct name should be followed by an open curly brace."))
  {
    iterator = find_next_token (Token_Type::Semicolon);
    return;
  }
  ...
  if (!expect_token (Token_Type::Close_Curly_Brace, "Expected a closing curly brace" ))
  {
    iterator = find_next_token (Token_Type::Semicolon);
    return;
  }
...
std::tuple<Variable::Type, unsigned int, std::string> Parser::determine_variable_type()
{
  unsigned int size = 1;
  ...
  else if (iterator->string.find("vec") != std::string::npos)
  {
    if (iterator->string.length() != 4 && iterator->string.length() != 5)
      register_error ("Vector type should be of the format TvecN where T is the type and N is the number of components.");
...
{
  valid = false;
  errors.push_back ("\nError parsing" + path.string() +
                    "at line: " + std::to_string(iterator->line_number) +
                    '\n' + message + '\n');
}

bool Parser::is_vertex_shader(std::filesystem::path const &path) const
{
  return path.has_extension() && path.extension().string().find("vert") != std::string::npos;
}

bool Parser::is_fragment_shader(std::filesystem::path const &path) const
{
  return path.has_extension() && path.extension().string().find("frag") != std::string::npos;
}
...
std::vector <std::unique_ptr <Uniform>> Parser::get_uniforms () const
{
  ...
  for (std::pair<const std::string, std::unique_ptr <Variable>> const &uniform : uniforms)
  {
    std::vector <std::unique_ptr <Uniform>> properties = uniform.second->get_properties ();

    for (std::unique_ptr <Uniform> const &property : properties)
      if (uniforms.find (property->get_name()) != uniforms.end())
        std::cerr << "Uniform " << property->get_name() << " is present twice.";

    uniform_variables.insert (uniform_variables.end(),
